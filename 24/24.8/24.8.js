...
const [notesList, noteForm, noteButton, eventsList, picassoContainer, selectSlice] = [
  'notes-list', 'note-form', 'note-button', 'events-list', 'picasso-container', 'select-slice'].map((id) => document.getElementById(id));
...
console.log('notes', notes.selectSlice(store.getState()));
...
selectSlice.addEventListener('click', () => {
  console.log(notes.selectSlice(store.getState()));
}, false);
